...
avg_bias_mat_baseline = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier)) # the mean of bias cell-wise (mean of mean bias)
avg_bias_sd_mat_baseline = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier)) # the sd of the mean of bias cell-wise
avg_value_mat_baseline = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier)) # posterior values of 12 cells, for each value of p

# bias for ar model
avg_bias_mat_ar = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier)) # the mean of bias cell-wise (mean of mean bias)
avg_bias_sd_mat_ar = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier)) # the sd of the mean of bias cell-wise
avg_value_mat_ar = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier)) # posterior values of 12 cells, for each value of p

mean_of_medians_ar = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier)) # the mean of median bias cell-wise
mean_of_medians_sd_ar = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier))

# bias for rw model
avg_bias_mat_rw = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier)) # the mean of bias cell-wise (mean of mean bias)
avg_bias_sd_mat_rw = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier)) # the sd of the mean of bias cell-wise
avg_value_mat_rw = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier)) # posterior values of 12 cells, for each value of p

mean_of_medians_rw = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier)) # the mean of median bias cell-wise
mean_of_medians_sd_rw = matrix(0, length(r), age_grouping_multiplier * (4* income_multiplier))
...
for (p in r) {
  ...
  # 10% quantile ridgeplots ----------------------------------------------------------------------------------
  
  quantile10_list_baseline_melted = melt(quantile10_list_baseline)
  quantile10_list_baseline_melted[,2] = factor(paste0(quantile10_list_baseline_melted[,2], " BL"),
                                                  levels = unique(paste0(quantile10_list_baseline_melted[,2], " BL")))
  quantile10_list_baseline_melted$Model = "Baseline"
  
  quantile10_list_ar_melted = melt(quantile10_list_ar)
  quantile10_list_ar_melted[,2] = factor(paste0(quantile10_list_ar_melted[,2], " AR"),
                                               levels = unique(paste0(quantile10_list_ar_melted[,2], " AR")))
  quantile10_list_ar_melted$Model = "Autoregressive"
  
  quantile10_list_rw_melted = melt(quantile10_list_rw)
  quantile10_list_rw_melted[,2] = factor(paste0(quantile10_list_rw_melted[,2], " RW"),
                                         levels = unique(paste0(quantile10_list_rw_melted[,2], " RW")))
  quantile10_list_rw_melted$Model = "Random walk"
  
  quantile10_df_combined = rbind(quantile10_list_baseline_melted, 
                                 quantile10_list_ar_melted,
                                 quantile10_list_rw_melted)
    
  colnames(quantile10_df_combined)[1:3] = c("n", "Type", "Preference")
  quantile10_df_combined$Model = factor(quantile10_df_combined$Model)
  quantile10_df_combined$Type = factor(quantile10_df_combined$Type,
                                       level = levels(quantile10_df_combined$Type)[correct_levels]) # change order of levels
  quantile10_df_combined$Model = factor(quantile10_df_combined$Model, 
                                        level = c("Random walk", "Autoregressive", "Baseline"))
  
  png(paste0("quantile10_",
             age_grouping_multiplier,
             "_",
             income_multiplier,
             "_",
  ...
  plot(ggplot(quantile10_df_combined, 
    ...
    ggtitle(paste0("10% quantiles of posterior linear predictors for each age cat. histograms.\n Runs=", runs,
                   ", Num. age groups=", age_grouping_multiplier,
                   ", Response prob. of old=", p)))
  
  dev.off()
  
  # 90% quantile ridgeplots ----------------------------------------------------------------------------------
  
  quantile90_list_baseline_melted = melt(quantile90_list_baseline)
  quantile90_list_baseline_melted[,2] = factor(paste0(quantile90_list_baseline_melted[,2], " BL"),
                                               levels = unique(paste0(quantile90_list_baseline_melted[,2], " BL")))
  quantile90_list_baseline_melted$Model = "Baseline"
  
  quantile90_list_ar_melted = melt(quantile90_list_ar)
  quantile90_list_ar_melted[,2] = factor(paste0(quantile90_list_ar_melted[,2], " AR"),
                                         levels = unique(paste0(quantile90_list_ar_melted[,2], " AR")))
  quantile90_list_ar_melted$Model = "Autoregressive"
  
  quantile90_list_rw_melted = melt(quantile90_list_rw)
  quantile90_list_rw_melted[,2] = factor(paste0(quantile90_list_rw_melted[,2], " RW"),
                                         levels = unique(paste0(quantile90_list_rw_melted[,2], " RW")))
  quantile90_list_rw_melted$Model = "Random walk"
  
  quantile90_df_combined = rbind(quantile90_list_baseline_melted, 
                                 quantile90_list_ar_melted,
                                 quantile90_list_rw_melted)
  
  colnames(quantile90_df_combined)[1:3] = c("n", "Type", "Preference")
  quantile90_df_combined$Model = factor(quantile90_df_combined$Model)
  quantile90_df_combined$Type = factor(quantile90_df_combined$Type,
                                       level = levels(quantile90_df_combined$Type)[correct_levels]) # change order of levels
  quantile90_df_combined$Model = factor(quantile90_df_combined$Model, 
                                        level = c("Random walk", "Autoregressive", "Baseline"))
  
  png(paste0("quantile90_",
             age_grouping_multiplier,
             "_",
             income_multiplier,
             "_",
  ...
  plot(ggplot(quantile90_df_combined, 
         aes(x = Preference, y = Type, fill=Model)) + 
    geom_density_ridges2(alpha=0.5, 
                         quantile_lines = TRUE, 
                         quantiles = c(0.1, 0.5, 0.9), 
    ...
    ggtitle(paste0("90% quantiles of posterior linear predictors for each age cat. histograms.\n Runs=", runs,
                   ", Num. age groups=", age_grouping_multiplier,
                   ", Response prob. of old=", p)))
  ...
  median_quantile_list_baseline_melted[,2] = factor(paste0(median_quantile_list_baseline_melted[,2], " BL"),
                                                    levels = unique(paste0(median_quantile_list_baseline_melted[,2], " BL")))
  median_quantile_list_baseline_melted$Model = "Baseline"
  
  median_quantile_list_ar_melted = melt(median_quantile_list_ar)
  median_quantile_list_ar_melted[,2] = factor(paste0(median_quantile_list_ar_melted[,2], " AR"),
                                              levels = unique(paste0(median_quantile_list_ar_melted[,2], " AR")))
  median_quantile_list_ar_melted$Model = "Autoregressive"
  
  median_quantile_list_rw_melted = melt(median_quantile_list_rw)
  median_quantile_list_rw_melted[,2] = factor(paste0(median_quantile_list_rw_melted[,2], " RW"),
                                              levels = unique(paste0(median_quantile_list_rw_melted[,2], " RW")))
  median_quantile_list_rw_melted$Model = "Random walk"
  
  median_quantile_df_combined = rbind(median_quantile_list_baseline_melted, 
                                      median_quantile_list_ar_melted,
                                      median_quantile_list_rw_melted)
  
  colnames(median_quantile_df_combined)[1:3] = c("n", "Type", "Preference")
  median_quantile_df_combined$Model = factor(median_quantile_df_combined$Model)
  median_quantile_df_combined$Type = factor(median_quantile_df_combined$Type,
                                            level = levels(median_quantile_df_combined$Type)[correct_levels]) # change order of levels
  median_quantile_df_combined$Model = factor(median_quantile_df_combined$Model, 
                                             level = c("Random walk", "Autoregressive", "Baseline"))
  ...
  plot(ggplot(median_quantile_df_combined, 
         aes(x = Preference, y = Type, fill=Model)) + 
    geom_density_ridges2(alpha=0.5, 
                         quantile_lines = TRUE, 
                         quantiles = c(0.1, 0.5, 0.9), 
    ...
    ggtitle(paste0("median of posterior linear predictors for each age cat. histograms.\n Runs=", runs,
                   ", Num. age groups=", age_grouping_multiplier,
                   ", Response prob. of old=", p)))
  ...
  mean_list_baseline_melted[,2] = factor(paste0(mean_list_baseline_melted[,2], " BL"),
                                         levels = unique(paste0(mean_list_baseline_melted[,2], " BL")))
  mean_list_baseline_melted$Model = "Baseline"
  
  mean_list_ar_melted = melt(mean_list_ar)
  mean_list_ar_melted[,2] = factor(paste0(mean_list_ar_melted[,2], " AR"),
                                   levels = unique(paste0(mean_list_ar_melted[,2], " AR")))
  mean_list_ar_melted$Model = "Autoregressive"
  
  mean_list_rw_melted = melt(mean_list_rw)
  mean_list_rw_melted[,2] = factor(paste0(mean_list_rw_melted[,2], " RW"),
                                   levels = unique(paste0(mean_list_rw_melted[,2], " RW")))
  mean_list_rw_melted$Model = "Random walk"
  
  mean_df_combined = rbind(mean_list_baseline_melted, 
                           mean_list_ar_melted,
                           mean_list_rw_melted)
  
  colnames(mean_df_combined)[1:3] = c("n", "Type", "Preference")
  mean_df_combined$Model = factor(mean_df_combined$Model)
  mean_df_combined$Type = factor(mean_df_combined$Type,
                                 level = levels(mean_df_combined$Type)[correct_levels]) # change order of levels
  mean_df_combined$Model = factor(mean_df_combined$Model, 
                                  level = c("Random walk", "Autoregressive", "Baseline"))
  ...
  plot(ggplot(mean_df_combined, 
    ...
    ggtitle(paste0("Mean of posterior linear predictors for each age cat. histograms.\n Runs=", runs,
                   ", Num. age groups=", age_grouping_multiplier,
                   ", Response prob. of old=", p)))
...
if (use_biascolourscheme==TRUE) {
    ...
    plot(
      ggplot(bias_facet_df, aes(x=prob_sampling_old, y=value, group=cell)) + 
        geom_line(aes(col=as.factor(age_cat)), show.legend = TRUE, size = 2) +
